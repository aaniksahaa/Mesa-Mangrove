...
const initGUI = function (model_params) {
  ...
  const addSliderInput = function (param, obj) {
    ...
    let chunks = obj.description.split(";");
    const desc = chunks[0];
    const color = chunks.length > 1 ? chunks[1].trim() : "";
    ...
    div.innerHTML = `
      <p>
        <span id='${tooltipID}' ${tooltip} data-bs-toggle='tooltip' data-bs-placement='top' class='badge bg-${type}'>
          ${obj.name}
        </span>
      </p>
      <div style="height: 10px;"></div>
      <input id='${domID}' type='text' class="custom-width" data-slider-handle="square" />
