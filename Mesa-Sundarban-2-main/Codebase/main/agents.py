...
class Mangrove_Fisher(mesa.Agent):
    def __init__(self, id, model, init_catching_capacity):
        ...
        self.inLoan = 0
    def switch_to_farmer(self):
        # Logic to transform this agent into a Farmer
        self.type = Type.FARMER
        self.crop_production_capacity = self.catching_capacity  # Assume conversion ratio
        self.catching_capacity = 0
        # self.model.farmers.append(self)  # Add to farmers list if maintained separately
        # self.model.fishermen.remove(self)  # Remove from fishermen list if maintained separately

    def switch_to_bawali(self):
        # Logic to transform this agent into a Bawali
        self.type = Type.BAWALI
        self.extraction_capacity = self.catching_capacity  # Assume conversion ratio
        self.catching_capacity = 0
        # self.model.bawalis.append(self)
        # self.model.fishermen.remove(self)    
    def step(self):
        # needed parameters
