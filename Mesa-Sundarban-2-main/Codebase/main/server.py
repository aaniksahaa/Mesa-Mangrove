...
class MyTextElement(TextElement):
    ...
    def render(self, model):
        ...
        # Get the current counts of each agent type
        num_fishermen = model.count_mangrove_fishermen()
        num_bawali = model.count_bawali()
        num_farmers = model.count_farmers()
        ...
        if "WARNING: High Fishermen Debt! Provide financial aid and alternative livelihood training." in restrict_forest_message:  # Reference to warning messages
             model.handle_fishermen_debt()  # Call the method on the model  
        agent_counts_html = f"""
        <div style='text-align: center; margin-top: 20px;'>
            <h3>Current Agent Counts</h3>
            <p>Mangrove Fishermen: {num_fishermen}</p>
            <p>Bawali: {num_bawali}</p>
            <p>Farmers: {num_farmers}</p>
        </div>
        """       
        # Regal HTML Output
        return f"""
        ...
        ">
            ...
            <div style="min-height: 200px; max-height: auto; overflow-y: auto; background-color: #fff5ee; padding: 10px; border: 1px solid #deb887; border-radius: 8px;">
