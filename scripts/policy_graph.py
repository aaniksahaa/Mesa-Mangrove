...
common_path = './statistics/saved/policy_data'

def get_filename(ind):
    return f"policy_{ind}_moderate.csv"
...
for column in columns_to_plot:
    ...
    for i in range(3):
        ...
        plt.plot(df.index, df[column], label=f'Policy {ind}', color=COLORS[i], linewidth=2)
    ...
    output_filename = f'./plots/policy/{column.replace(" ", "_").lower()}_vs_time.png'
...
print(all_metrics["Current Bawali Count"])
