...
common_path = './statistics/saved/rogue_data'

rogue_pcts = [0,0.5,1]

def get_filename(ind):
    return f"rogue_pct_{rogue_pcts[ind-1]}_moderate_policy_3.csv"
...
for column in columns_to_plot:
    ...
    for i in range(3):
        ...
        plt.plot(df.index, df[column], label=f'Rogue {int(rogue_pcts[ind-1]*100)}%', color=COLORS[i], linewidth=2)

    # Setting title and labels with a sophisticated font
    plt.title(f'{column} vs Time (POlicy 3)', fontsize=20, fontweight='bold', fontname='serif')
    ...
    output_filename = f'./plots/rogue/{column.replace(" ", "_").lower()}_vs_time.png'
